...
import game.GameTestFixturesUtils
...
class GameTest :
    FunSpec({
        val gameTestFixturesUtils = GameTestFixturesUtils()
        val playerTestFixturesUtils = PlayerTestFixturesUtils()
        val player = playerTestFixturesUtils.createPlayer()

        test("Game 생성시 Status는 BEFORE_START") {
            val game = gameTestFixturesUtils.createGame(player)
        ...
        test("참가자 2명인 경우") {
            ...
            val player2 = playerTestFixturesUtils.createPlayer()
        ...
        test("참가자는 2명 이상 불가능 하다.") {
            ...
            val player3 = playerTestFixturesUtils.createPlayer()
            shouldThrow<CustomException> {
        ...
        test("게임이 시작하지 않은경우, 끝낼 수 없다.") {
            val game = gameTestFixturesUtils.createGame(player)
            game.addPlayer(playerTestFixturesUtils.createPlayer())
